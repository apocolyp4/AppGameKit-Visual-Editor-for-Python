...
def parse_entity_data(self, entity, data):
    ...
    entity_property_found = False

    if data.tag == "type":
        entity.type = data.value
        entity_property_found = 1

    elif entity.type == "sprite":
        entity_property_found = set_sprite_entity_data(entity, data)

    elif entity.type == "text":
        entity_property_found = set_text_entity_data(entity, data)

    elif entity.type == "particles":
        entity_property_found = set_particles_entity_data(entity, data)

    elif entity.type == "button":
        entity_property_found = set_button_entity_data(entity, data)

    elif entity.type == "editbox":
        entity_property_found = set_editbox_entity_data(entity, data)

    if entity_property_found == False:
        if data.tag == "scene":
...
def set_sprite_entity_data(entity, data):
    if data.tag == "image":
        entity.image = agk.replace_string(data.value, "media/", "", 1)
        return True
...
def set_text_entity_data(entity, data):
    if data.tag == "alignment":
...
def set_particles_entity_data(entity, data):

    if data.tag == "particle image":
...
def set_button_entity_data(entity, data):
    if data.tag == "button text":
...
def set_editbox_entity_data(entity, data):
    if data.tag == "edit size":
